...
Page({
  ...
  onLoad: function (options) {
    ...
    var self = this;
    wx.request({
      url: 'https://citgroup.cn/findById2',
      data: {
      ...
      success: function (res) {
        ...
        if(res.status == "SUCCESS"){
          if(res.data && res.data.article && res.data.article.articleContent){
            var articleInfo = res.data.article;
            var txt = res.data.article.articleContent;
            txt = txt.replace(/\/?u\//g,"http://www.citgroup.cn/u/");
            WxParse.wxParse('article', 'html', txt, self, 5);
            self.setData({
              title: articleInfo.title,
              time: articleInfo.relaseDate
             }
            );
          }
        }
